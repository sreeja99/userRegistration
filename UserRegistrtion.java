...
public class UserRegistrtion {
	...
	private String email;
     public boolean firName(String firstName) {
		Matcher matcher=pattern.matcher(firstName);//checking first name
		return (matcher.matches());
	}
	public boolean lastName(String lastName) { 
		Matcher matcher1=pattern1.matcher(lastName);//checking last name
		return (matcher1.matches());
	}
	public boolean email(String email) {
		Matcher matcher2=pattern2.matcher(email);//checking email
		return (matcher2.matches());
	}
	public  boolean mobile(String mobile) {
		Matcher matcher3 = pattern3.matcher(mobile);//checking mobile
		 return(matcher3.matches());
	}
	public boolean password(String password) {
		Matcher matcher4 = pattern4.matcher(password);//checking password
